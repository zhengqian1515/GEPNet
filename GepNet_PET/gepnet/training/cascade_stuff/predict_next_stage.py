...
import argparse
from gepnet.preprocessing.preprocessing import resample_data_or_seg
from batchgenerators.utilities.file_and_folder_operations import maybe_mkdir_p
import gepnet
from gepnet.run.default_configuration import get_default_configuration
from multiprocessing import Pool

from gepnet.training.model_restore import recursive_find_python_class
from gepnet.training.network_training.Trainer_acdc import Trainer_acdc
...
if __name__ == "__main__":
    ...
    trainer_class = recursive_find_python_class(
        [join(gepnet.__path__[0], "training", "network_training")],
        trainerclass,
        "gepnet.training.network_training",
    )

    if trainer_class is None:
        raise RuntimeError(
            "Could not find trainer class in gepnet.training.network_training"
        )
    else:
        assert issubclass(
            trainer_class, Trainer_acdc
        ), "network_trainer was found but is not derived from nnFormerTrainer"
