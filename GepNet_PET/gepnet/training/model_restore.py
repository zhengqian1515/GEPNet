...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None, folder=None):
    ...
    gepnet.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    if len(init) > 10:
        ...
        del init[2]
        del init[-2]
    search_in = join(gepnet.__path__[0], "training", "network_training")
    tr = recursive_find_python_class(
        [search_in], name, current_module="gepnet.training.network_training"
    )
    ...
    if tr is None:
        raise RuntimeError(
            "Could not find the model trainer specified in checkpoint in gepnet.trainig.network_training. If it "
            "is not located there, please move it or change the code of restore_model. Your model "
            "trainer can be located in any directory within gepnet.trainig.network_training (search is recursive)."
            "\nDebug info: \ncheckpoint file: %s\nName of trainer: %s "
            % (checkpoint, name)
        )
