...
from gepnet.experiment_planning.common_utils import split_4d_nifti
from gepnet.paths import (
    nnFormer_raw_data,
    nnFormer_cropped_data,
    preprocessing_output_dir,
)
from gepnet.preprocessing.cropping import ImageCropper
...
def split_4d(
    ...
    output_folder = join(
        nnFormer_raw_data, "Task%03.0d_" % overwrite_task_output_id + task_name
    )
...
def plan_and_preprocess(
    ...
    from gepnet.experiment_planning.experiment_planner_baseline_2DUNet import (
        ExperimentPlanner2D,
    )
    ...
    shutil.copy(
        join(nnFormer_raw_data, task_string, "dataset.json"),
        preprocessing_output_dir_this_task_train,
    )
