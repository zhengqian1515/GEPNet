...
from gepnet.experiment_planning.utils import crop
from gepnet.paths import *
import shutil
from gepnet.utilities.task_name_id_conversion import convert_id_to_task_name
from gepnet.preprocessing.sanity_checks import verify_dataset_integrity
...
def main():
    ...
    search_in = join(gepnet.__path__[0], "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class(
            [search_in], planner_name3d, current_module="gepnet.experiment_planning"
        )
        if planner_3d is None:
            raise RuntimeError(
                "Could not find the Planner class %s. Make sure it is located somewhere in "
                "gepnet.experiment_planning" % planner_name3d
            )
    else:
        planner_3d = None
    ...
    if planner_name2d is not None:
        planner_2d = recursive_find_python_class(
            [search_in], planner_name2d, current_module="gepnet.experiment_planning"
        )
        if planner_2d is None:
            raise RuntimeError(
                "Could not find the Planner class %s. Make sure it is located somewhere in "
                "gepnet.experiment_planning" % planner_name2d
            )
    else:
        planner_2d = None
