...
import numpy as np
from gepnet.experiment_planning.experiment_planner_baseline_3DUNet_v21 import ExperimentPlanner3D_v21
from gepnet.experiment_planning.experiment_planner_baseline_2DUNet_v21 import ExperimentPlanner2D_v21
...
class ExperimentPlanner3D_v21_customTargetSpacing_4x4x4(ExperimentPlanner3D_v21):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        super(ExperimentPlanner3D_v21, self).__init__(folder_with_cropped_data, preprocessed_output_folder)
        # we change the data identifier and plans_fname. This will make this experiment planner save the preprocessed
        # data in a different folder so that they can co-exist with the default (ExperimentPlanner3D_v21). We also
        # create a custom plans file that will be linked to this data
        self.data_identifier = "nnFormerData_plans_v2.1_trgSp_4x4x4"
        self.plans_fname = join(self.preprocessed_output_folder,
                                "nnFormerPlansv2.1_trgSp_4x4x4_plans_3D.pkl")

    def get_target_spacing(self):
        # simply return the desired spacing as np.array
        return np.array([4., 4., 4.])  # make sure this is float!!!! Not int!
    ...
    def get_properties_for_stage(
        ...
        """
        plans = super(ExperimentPlanner3D_v21_customTargetSpacing_4x4x4, self).get_properties_for_stage(
            current_spacing, original_spacing, original_shape, num_cases, num_modalities, num_classes
        )
        plans["patch_size"] = [128, 128, 128]
        return plans
...
class ExperimentPlanner2D_v21_customTargetSpacing_1x1x1(ExperimentPlanner2D_v21):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        ...
        # data in a different folder so that they can co-exist with the default (ExperimentPlanner3D_v21). We also
        # create a custom plans file that will be linked to this data
        self.data_identifier = "nnFormerData2D_plans_v2.1_trgSp_1x1x1"
        self.plans_fname = join(self.preprocessed_output_folder,
                                "nnFormerPlansv2.1_trgSp_1x1x1_plans_2D.pkl")
...
class ExperimentPlanner2D_v21_customTargetSpacing_4x4x4(ExperimentPlanner2D_v21):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder):
        super(ExperimentPlanner2D_v21, self).__init__(folder_with_cropped_data, preprocessed_output_folder)
        # we change the data identifier and plans_fname. This will make this experiment planner save the preprocessed
        # data in a different folder so that they can co-exist with the default (ExperimentPlanner3D_v21). We also
        # create a custom plans file that will be linked to this data
        self.data_identifier = "nnFormerData2D_plans_v2.1_trgSp_4x4x4"
        self.plans_fname = join(self.preprocessed_output_folder,
                                "nnFormerPlansv2.1_trgSp_4x4x4_plans_2D.pkl")
    ...
    def get_properties_for_stage(
        ...
        plans = super(ExperimentPlanner2D_v21_customTargetSpacing_4x4x4, self).get_properties_for_stage(
            current_spacing, original_spacing, original_shape, num_cases, num_modalities, num_classes
        )
        plans["patch_size"] = [512, 512]
        return plans