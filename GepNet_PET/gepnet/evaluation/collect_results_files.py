...
def crawl_and_copy(current_folder, out_folder, prefix="fabian_", suffix="ummary.json"):
    ...
    if current_folder.find("fold0") != -1:
        for fl in f:
            shutil.copy(
                os.path.join(current_folder, fl), os.path.join(out_folder, prefix + fl)
            )
    for su in s:
        if prefix == "":
            add = su
            ...
            add = "__" + su
        crawl_and_copy(
            os.path.join(current_folder, su), out_folder, prefix=prefix + add
        )
...
if __name__ == "__main__":
    ...
    from gepnet.evaluation.add_mean_dice_to_json import run_in_folder
