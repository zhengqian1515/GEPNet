...
from gepnet.evaluation.add_mean_dice_to_json import foreground_mean
from gepnet.evaluation.evaluator import evaluate_folder
from gepnet.evaluation.model_selection.ensemble import ensemble
from gepnet.paths import network_training_output_dir
import numpy as np
from subprocess import call
from gepnet.postprocessing.consolidate_postprocessing import (
    consolidate_folds,
    collect_cv_niftis,
)
from gepnet.utilities.folder_names import get_output_folder_name
from gepnet.paths import (
...
def find_task_name(folder, task_id):
    ...
    assert len(candidates) > 0, "no candidate for Task id %d found in folder %s" % (
        task_id,
        folder,
    )
    assert (
        len(candidates) == 1
    ), "more than one candidate for Task id %d found in folder %s" % (task_id, folder)
    return candidates[0]
...
def get_foreground_mean(results):
    ...
    dice_scores = [
        results_mean[i]["Dice"] for i in results_mean.keys() if i != "0" and i != "mean"
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        usage="This is intended to identify the best model based on the five fold "
    ...
    parser.add_argument(
        ...
        help="nnFormerTrainer class for cascade model. Default: %s"
        % default_cascade_trainer,
    )
    parser.add_argument(
        "-pl",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=(0, 1, 2, 3, 4),
        help="Use this if you have non-standard " "folds. Experienced users only.",
    )
    parser.add_argument(
        "--disable_ensembling",
        required=False,
    ...
    for t in tasks:
        ...
        for m in models:
            ...
            if disable_postprocessing:
                ...
                if not isfile(join(cv_niftis_folder, "summary.json")):
                    ...
                    collect_cv_niftis(
                        output_folder, cv_niftis_folder, validation_folder, folds
                    )

                    niftis_gt = subfiles(
                        join(output_folder, "gt_niftis"), suffix=".nii.gz", join=False
                    )
                    niftis_cv = subfiles(cv_niftis_folder, suffix=".nii.gz", join=False)
                    if not all([i in niftis_gt for i in niftis_cv]):
                        raise AssertionError(
                            "It does not seem like you trained all the folds! Train "
                            "all folds first! There are %d gt niftis in %s but only "
                            "%d predicted niftis in %s"
                            % (len(niftis_gt), niftis_gt, len(niftis_cv), niftis_cv)
                        )
                    ...
                    evaluate_folder(
                        join(output_folder, "gt_niftis"), cv_niftis_folder, classes
                    )
                ...
                assert isdir(cv_niftis_folder), (
                    "Folder with niftis from CV missing, expected: %s"
                    % cv_niftis_folder
                )
        ...
        if not disable_ensembling:
            ...
            if len(valid_models) > 1:
                for m1, m2 in combinations(valid_models, 2):
                    ...
                    summary_file = join(
                        output_folder_base, "ensembled_raw", "summary.json"
                    )
                    foreground_mean(summary_file)
                    all_results[ensemble_name] = load_json(summary_file)["results"][
                        "mean"
                    ]
        ...
        for k, v in results.items():
            if v == best:
                ...
                if k.startswith("ensemble"):
                    ...
                    predict_str += (
                        "nnFormer_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr "
                        + tr
                        + " -ctr "
                        + trc
                        + " -m "
                    ...
                    predict_str += (
                        "nnFormer_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL2 -tr "
                        + tr
                        + " -ctr "
                        + trc
                        + " -m "
                    ...
                    if not disable_postprocessing:
                        predict_str += (
                            "nnFormer_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER -pp "
                            + join(
                                network_training_output_dir,
                                "ensembles",
                                id_task_mapping[t],
        ...
        summary_folder = join(
            network_training_output_dir, "ensembles", id_task_mapping[t]
        )
        ...
        num_classes = len(
            [i for i in all_results[best_model].keys() if i != "mean" and i != "0"]
        )
