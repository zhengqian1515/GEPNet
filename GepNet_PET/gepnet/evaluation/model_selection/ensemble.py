...
from gepnet.configuration import default_num_threads
from gepnet.evaluation.evaluator import aggregate_scores
from gepnet.inference.segmentation_export import save_segmentation_nifti_from_softmax
from gepnet.paths import network_training_output_dir, preprocessing_output_dir
from gepnet.postprocessing.connected_components import determine_postprocessing
...
def ensemble(
    ...
    plans = load_pickle(
        join(training_output_folder1, "plans.pkl")
    ...
    for f in folds:
        validation_folder_net1 = join(
            training_output_folder1, "fold_%d" % f, validation_folder
        )
        validation_folder_net2 = join(
            training_output_folder2, "fold_%d" % f, validation_folder
        )

        if not isdir(validation_folder_net1):
            raise AssertionError(
                "Validation directory missing: %s. Please rerun validation with `nnFormer_train CONFIG TRAINER TASK FOLD -val --npz`"
        ...
        if not isfile(join(validation_folder_net1, "summary.json")):
            raise AssertionError(
                "Validation directory incomplete: %s. Please rerun validation with `nnFormer_train CONFIG TRAINER TASK FOLD -val --npz`"
                % validation_folder_net1
            )
        ...
        patient_identifiers1_npz = [
            i[:-4] for i in subfiles(validation_folder_net1, False, None, "npz", True)
        ]
        patient_identifiers2_npz = [
            i[:-4] for i in subfiles(validation_folder_net2, False, None, "npz", True)
        ]
        ...
        patient_identifiers1_nii = [
            i[:-7]
            for i in subfiles(
                validation_folder_net1, False, None, suffix="nii.gz", sort=True
            )
        ...
        patient_identifiers2_nii = [
            i[:-7]
            for i in subfiles(
                validation_folder_net2, False, None, suffix="nii.gz", sort=True
            )
            if not i.endswith("noPostProcess.nii.gz")
            and not i.endswith("_postprocessed.nii.gz")
        ]
        ...
        assert all(
            [i == j for i, j in zip(patient_identifiers1_npz, patient_identifiers2_npz)]
    ...
    if not isfile(join(output_folder, "summary.json")) and len(out_files) > 0:
        aggregate_scores(
            ...
            json_task=task,
            json_name=task + "__" + output_folder_base.split("/")[-1],
    ...
    if allow_ensembling and not isfile(join(output_folder_base, "postprocessing.json")):
        ...
        json_out = load_json(
            join(output_folder_base, "ensembled_postprocessed", "summary.json")
        )
