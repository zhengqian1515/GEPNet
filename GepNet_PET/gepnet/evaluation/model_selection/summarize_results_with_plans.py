...
import os
from gepnet.evaluation.model_selection.summarize_results_in_one_json import summarize
...
def write_plans_to_file(
    ...
    f.write(
        ";%s" % list_to_string(a["plans_per_stage"][stages[stage]]["current_spacing"])
    )
    f.write(
        ";%s" % list_to_string(a["plans_per_stage"][stages[stage]]["original_spacing"])
    )
...
if __name__ == "__main__":
    summarize(
        ...
        output_dir=join(network_training_output_dir, "summary_fold0"),
        folds=(0,),
    )
    ...
    with open("summary.csv", "w") as f:
        f.write(
            "identifier;stage;batch_size;num_pool_per_axis;patch_size;patch_size(mm);median_patient_size_in_voxels;median_patient_size_in_mm;current_spacing;original_spacing;pool_op_kernel_sizes;conv_kernel_sizes;patient_dc;global_dc\n"
        )
        for i in task_ids:
            for nnformer in nnformers:
                try:
                    ...
                    if isdir(summary_folder):
                        summary_files = subfiles(
                            summary_folder,
                            join=False,
                            prefix="Task%03.0d_" % i,
                            suffix=".json",
                            sort=True,
                        )
                        for s in summary_files:
                            ...
                            expected_output_folder = join(
                                base_dir, nnformer, tmp[1], tmp[0], tmp[2].split(".")[0]
                            )
                            name = (
                                tmp[0]
                                + "__"
                            ...
                            global_dice_json = join(
                                ...
                                tmp[2].split(".")[0],
                                "fold_0",
                                "validation_tiledTrue_doMirror_True",
                                "global_dice.json",
                            )
                            ...
                            if num_stages > 1 and tmp[1] == "3d_fullres":
                                stage = 1
                            elif (num_stages == 1 and tmp[1] == "3d_fullres") or tmp[
                                1
                            ] == "3d_lowres":
                                stage = 0
                            else:
