    ...
    for model in models:
        for t in tasks:
            ...
            if not isdir(join(network_training_output_dir, model)):
                continue
            task_name = subfolders(
                join(network_training_output_dir, model),
                prefix="Task%03.0d" % t,
                join=False,
            )
            if len(task_name) != 1:
                ...
                for v in validation_folders:
                    ...
                    for fld in folds:
                        ...
                        metrics_tmp = load_json(
                            join(validation_folder, "summary.json")
                        )["results"]["mean"]
                    ...
                    if ok:
                        ...
                        json_out["description"] = (
                            model + " " + task_name + " all folds summary"
                        )
                        json_out["name"] = (
                            model + " " + task_name + " all folds summary"
                        )
                        ...
                        save_json(
                            json_out, join(out_dir, "summary_allFolds__%s.json" % v)
                        )
                        save_json(
                            json_out,
                            join(
                                output_dir,
                                "%s__%s__%s__%s.json" % (task_name, model, trainer, v),
                            ),
                        )
            ...
            for trainer in model_trainers:
                ...
                for fld in folds:
                    ...
                    validation_folders += subfolders(
                        fold_output_dir, prefix="validation", join=False
                    )
                ...
                for v in validation_folders:
                    ...
                    for fld in folds:
                        ...
                        all_labels = [
                            i for i in list(mean_metrics.keys()) if i != "mean"
                        ]
                    ...
                    if ok:
                        ...
                        json_out["task"] = task_name
                        json_out["description"] = (
                            model + " " + task_name + "summary folds" + str(folds)
                        )
                        json_out["name"] = (
                            model + " " + task_name + "summary folds" + str(folds)
                        )
                        ...
                        save_json(
                            ...
                            join(
                                output_dir,
                                "%s__%s__%s__%s__%s.json"
                                % (task_name, model, trainer, v, fold_string),
                            ),
                        )
                        foreground_mean2(
                            join(
...
def foreground_mean2(filename):
    with open(filename, "r") as f:
        res = json.load(f)
    class_ids = np.array(
        [int(i) for i in res["results"]["mean"].keys() if (i != "mean") and i != "0"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--task_ids", nargs="+", required=True, help="task id. can be 'all'"
    )
    parser.add_argument(
        "-f", "--folds", nargs="+", required=False, type=int, default=[0, 1, 2, 3, 4]
    )
    parser.add_argument(
        "-m",
        "--models",
    ...
    summarize2(
        ...
        models,
        folds=folds,
        output_dir=join(network_training_output_dir, "summary_jsons_new"),
    )
