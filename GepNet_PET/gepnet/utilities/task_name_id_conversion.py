...
def convert_id_to_task_name(task_id: int):
    ...
    if len(unique_candidates) == 0:
        raise RuntimeError(
            ...
            "Installation). Here are your currently defined folders:\ngepnet_preprocessed=%s\nRESULTS_"
            "FOLDER=%s\ngepnet_raw_data_base=%s\nIf something is not right, adapt your environemnt "
            "variables."
            % (
                task_id,
                os.environ.get("gepnet_preprocessed")
                if os.environ.get("gepnet_preprocessed") is not None
                else "None",
                os.environ.get("RESULTS_FOLDER")
                if os.environ.get("RESULTS_FOLDER") is not None
                else "None",
                os.environ.get("gepnet_raw_data_base")
                if os.environ.get("gepnet_raw_data_base") is not None
                else "None",
            )
        )
